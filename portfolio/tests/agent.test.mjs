...
test('comparisons link both flagship sources and followups preserve the last named project', async () => {
  const comparison = await generateReply('Compare the projects', []);
  assert.deepEqual(comparison.sources, ['ircc', 'bogdai']);
...
test('live request keeps credentials on the server, opts out of storage, and validates structured output', async () => {
  ...
  const result = await generateReply('Show the agent tooling', [], {
    ...
    fetcher: async (url, options) => {
      ...
      return { ok: true, json: async () => ({ status: 'completed', output: [{ type: 'message', content: [{ type: 'output_text', text: JSON.stringify({ answer: 'Explore the six-agent contract pipeline.', project: 'bogdai', contact: false }) }] }] }) };
    },
  });
